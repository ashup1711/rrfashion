...
export class InvoicesService {
  ...
  private async generatePdf(data: {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.text(`Date: ${order.createdAt.toLocaleDateString('en-IN')}`, rightColX, metaY, {
        width: 200,
        align: 'right',
      });
      doc.text(`Financial Year: ${financialYear}`, rightColX, metaY + 13, {
        width: 200,
        align: 'right',
      });
      ...
      for (let i = 0; i < order.items.length; i++) {
        ...
        xPos += colWidths.desc;
        doc.text(item.product.hsnCode || '-', xPos, rowY + 3, {
          width: colWidths.hsn,
          align: 'center',
        });
        xPos += colWidths.hsn;
        ...
        xPos += colWidths.qty;
        doc.text(
          `₹${item.unitPrice.toNumber().toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
          xPos,
          rowY + 3,
          { width: colWidths.rate, align: 'right' },
        );
        xPos += colWidths.rate;
        doc.text(
          `₹${item.totalPrice.toNumber().toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
          xPos,
          rowY + 3,
      ...
      doc
        ...
        .fill('#6B7280')
        .text('This is a computer-generated invoice. No signature required.', leftMargin, footerY, {
          align: 'center',
          width: pageWidth,
        });
  ...
  private async generateCreditNotePdf(data: {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        doc.text(
          `Address: ${addr.line1 || ''}${addr.city ? ', ' + addr.city : ''}${addr.state ? ', ' + addr.state : ''}`,
          leftMargin,
          custY + 36,
        );
      } catch {
